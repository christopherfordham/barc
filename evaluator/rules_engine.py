...
def summarise(results: list[dict]) -> dict:
    # Compute duty-level status, colour and human-readable notes
    has_violation = False
    min_flex = None
    notes = []

    for r in results:
        # Gather flex if present
        limits = r.get("limits", {})
        if "flex_minutes" in limits and isinstance(limits.get("flex_minutes"), (int, float)):
            fm = int(limits["flex_minutes"])
            if min_flex is None or fm < min_flex:
                min_flex = fm

        # Gather violations and info
        for v in r.get("violations", []) or []:
            has_violation = True
            # Prefer 'excess_minutes' if present, else -flex
            excess = None
            if isinstance(v, dict):
                excess = v.get("excess_minutes")
                if excess is None and "rule" in v and "title" in v:
                    # Fall back to negative flex if available
                    if "flex_minutes" in limits:
                        excess = max(0, -int(limits["flex_minutes"]))
            rule_src = r.get("rule_source") or v.get("rule_source") if isinstance(v, dict) else r.get("rule_source")
            rule_name = (v.get("title") if isinstance(v, dict) else "Rule issue") or v.get("rule")
            if excess is not None:
                notes.append({
                    "rule_source": rule_src,
                    "rule": rule_name,
                    "detail": f"Exceeded by {_fmt_hm(int(excess))}",
                    "category": "violation",
                    "color_code": "red"
                })
            else:
        ...
        # Informational notes (e.g., Within FDP)
        for inf in r.get("info", []) or []:
            if isinstance(inf, dict) and "flex_minutes" in inf:
                fm = int(inf["flex_minutes"])
                rule_src = r.get("rule_source")
                detail = "Within FDP"
                if fm is not None:
                    detail = f"Within FDP (flex {_fmt_hm(fm)})"
                notes.append({
                    "rule_source": rule_src,
                    "rule": inf.get("note","Info"),
                    "detail": detail,
                    "category": "info",
                    "color_code": "green" if fm is not None and fm > 30 else "amber" if 0 <= fm <= 30 else "red" if fm < 0 else "green"
                })
            elif isinstance(inf, dict):
                notes.append({
                    "rule_source": r.get("rule_source"),
                    "rule": inf.get("note","Info"),
                    "detail": inf.get("note","Info"),
                    "category": "info",
                    "color_code": "green"
                })

    # Determine overall colour
    if has_violation or (min_flex is not None and min_flex < 0):
        color = "red"
        overall = "Non-Compliant"
    elif min_flex is not None and 0 <= min_flex <= 30:
        color = "amber"
        overall = "At-Risk"
        # Add an explicit near-limit note if not already present
        notes.append({
            "rule_source": "BARC",
            "rule": "Near limit",
            "detail": f"Within {_fmt_hm(min_flex)} of a limit",
            "category": "warning",
            "color_code": "amber"
        })
    else:
    ...
    return {"overall_status": overall, "color_code": color, "notes": notes}
